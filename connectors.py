...
def fetch_from_opml(filename, max_days_ago):
    ...
    for feed_entry in opml.parse(filename)[0]:
        ...
        entries = [e for e in feed['entries'] if 'published' in e.keys() and (datetime.now(
        ) - dateutil.parser.parse(e['published']).replace(tzinfo=None)).days < max_days_ago]
    ...
    aggregate_entries = sorted(aggregate_entries, key=lambda x: (
        datetime.now() - dateutil.parser.parse(x['published']).replace(tzinfo=None)).days)
    ...
    for e_idx, e in enumerate(aggregate_entries):
        ...
        aggregate_titles[e_idx] = e.get(
            'author', '?') + ' | ' + e.get('title', '?')

    raw_contents = aggregate_contents.copy()
    aggregate_contents = [BeautifulSoup(
        e, 'html.parser').get_text() for e in aggregate_contents]

    data = {}
    for idx in range(len(aggregate_contents)):
        data[aggregate_titles[idx]] = [
            aggregate_contents[idx], raw_contents[idx], '']
...
def fetch_from_bookmarks(filename, bookmark_folder, max_days_ago):
    ...
    folder_element = [e for e in bookmark_soup.find_all(
        'h3') if re.compile(bookmark_folder).match(e.text)]
    bookmarks = folder_element[0].parent()[1].find_all('a')
    bookmarks = [e for e in bookmarks if (
        time.time() - float(e['add_date']) < max_days_ago * 60 * 60 * 24)]
    ...
    for idx in range(len(bookmark_contents)):
        data[bookmark_titles[idx]] = [
            bookmark_contents[idx], bookmark_raw[idx], '']
...
def fetch_from_epub(filename):
    ...
    content = ''.join([e.content.decode('utf-8')
                      for e in book.items if isinstance(e, epub.EpubHtml)])
    processed = BeautifulSoup(content, 'html.parser').get_text()

    data = {}
    data[book.get_metadata('DC', 'creator')[0][0] +
         ' | ' + book.title] = [processed, content, '']
...
def fetch_from_pdf(filename):
    ...
    if doc['metadata'].get('Author', '') != '':
        data[doc['metadata']['Author'] + ' | ' + doc['metadata']
             ['pdf:docinfo:title']] = [doc['content'], doc['content']]
    else:
        data[os.path.splitext(os.path.basename(filename))[0]] = [
            doc['content'], doc['content'], filename]
